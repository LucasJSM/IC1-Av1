...
def draw_grid(screen, distances, font):
    ...
    for (x1, y1), (x2, y2) in walls:
        start_pos = (x1 * TILE_SIZE + (TILE_SIZE if x2 > x1 or y2 > y1 else 0),
                     y1 * TILE_SIZE + (TILE_SIZE if y2 > y1 else 0))
        end_pos = (x2 * TILE_SIZE + (0 if x2 > x1 or y2 > y1 else TILE_SIZE),
                   y2 * TILE_SIZE + (0 if y2 > y1 else TILE_SIZE))
